...
def moveCircle():
    ...
    canvas.move(happy, 0, happyVelocity) # Move the circle right by 0 and down by 1 pixel : Simulating gravity
    ...
    # Checks if the top side of bird is above the bottom side of the top pipe.
        if (coordsOfHappy[0] < top_pipe_coords[2]  and coordsOfHappy[2] > top_pipe_coords[0] and
            coordsOfHappy[1] < top_pipe_coords[3]  and coordsOfHappy[3] > top_pipe_coords[1]) or \
            (coordsOfHappy[0] < bottom_pipe_coords[2]  and coordsOfHappy[2] > bottom_pipe_coords[0] and
            coordsOfHappy[1] < bottom_pipe_coords[3]  and coordsOfHappy[3] > bottom_pipe_coords[1]):
...
def gameOver():
    global gameIsRunning
    gameIsRunning = False
    game_over_text = canvas.create_text(400, 400, text="GAME OVER", fill="red", font=("Impact", 40))
    game_over_animation()
    ...
    #create a button
    game_over_btn = tk.Button(canvas, width=20, height=10, text = 'Play Again', bd = '5', padx = 0, pady=0, command = game_over_btn_click)
    game_over_btn_window = canvas.create_window(WIDTH / 2, HEIGHT / 2, anchor = 'center', window = game_over_btn)  
...
# Create a Button
btn = tk.Button(canvas, width=20, height=10, text = 'Start', bd = '5', padx = 0, pady=0, command = btn_click)
